...
idl/admin-authority.idl.spel-generated.rc3-testnet.json  # v0.2.0 / current testnet pins (v0.2.0 / spel 31e52c52)
```
...
- **public-testnet deploy + authority lifecycle (2026-06-27, corrected guest): proven on `https://testnet.lez.logos.co/`** under real consensus and `RISC0_DEV_MODE=0` — ProgramId/ImageID `338865e9…871554a` (base58 `4UARaVcJJoLxebFAobocsZyzpJ5TTUvvhRtFuHtuHypd`), deploy `79399225…b717dce`, `create_mint` `55908821…cf67746`, `create_holding` `8a37a8fb…ac668f`, `mint_to(60)` `daf5aa91…cfdb6a97`, `mint_to(40)` `ed07b29c…29b3cce` [accumulates → 100], `set_mint_authority(None)` `719123f9…508550e`, post-revoke `mint_to` `01604377…3d4fa` never included; mint-PDA readback `authority=None, supply=100, decimals=6` and holding `balance=100`. The two accumulating mints demonstrate variable supply on chain, and the post-revoke rejection is the authority guard (`require_authority`), not an init side effect — the holding already exists (`mut`). Re-verify read-only: `bash scripts/demo-testnet-live.sh verify`.
