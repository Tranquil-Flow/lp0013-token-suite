...
| CI green on default branch | Complete | GitHub Actions on the implementation repo | Hosted CI completed successfully after the standalone-localnet CI/preflight changes: `Rust workspace` and `Standalone LEZ local-sequencer e2e preflight` pass; manual self-hosted localnet and public-testnet jobs are correctly skipped on push and available via `workflow_dispatch`. Re-check the latest run after every push. |
