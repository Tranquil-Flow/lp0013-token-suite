...
| CI green on default branch | Must be checked before final PR update | `gh run list --limit 10 --json status,conclusion,workflowName,headBranch,headSha,createdAt,url` | Do not claim final PR readiness until latest default-branch deterministic CI is green after these changes are pushed. |
