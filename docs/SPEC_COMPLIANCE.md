...
Status: **on-chain verified (see banner).** Offline Rust authority suite is locally green (now including a repeated-mint / post-revoke-guard contract test), the SPEL IDL is regenerated for the corrected four-instruction surface (the rc1 and rc3 pin generations are byte-identical), and the guest source carries the corrected two-instruction minting model (`create_holding` + mutable `mint_to`). The corrected guest is **deployed and its authority lifecycle is verified on the public LEZ testnet** (2026-06-27; ProgramId/ImageID `32335764…b0a9ce`; two accumulating mints → supply 100, guard-rejected post-revoke mint) — re-verify read-only with `bash scripts/demo-testnet-live.sh verify`. The prior 2026-06-03 testnet run is the superseded pre-fix record (see `docs/LEZ_PROOF_LOG.md`). The final narrated demo video is recorded at https://youtu.be/rUgsCCPiQfo, and the standalone local-sequencer e2e path is included in CI configuration with retained real-run evidence from a prepared LEZ/RISC0 host.
...
- CI workflow: added at `.github/workflows/ci.yml`, including deterministic hosted gates, hosted localnet preflight, manual self-hosted standalone local-sequencer e2e, and manual public-testnet read-only verification.
- Deployed/tested on LEZ testnet: **done (corrected guest, 2026-06-27).** The corrected guest was deployed and exercised on the **public testnet** (`testnet.lez.logos.co`, `RISC0_DEV_MODE=0`; ProgramId/ImageID `32335764…b0a9ce`): create_mint → create_holding → mint_to(60) → mint_to(40) [accumulates → 100] → set_mint_authority(None) → guard-rejected post-revoke mint. Two accumulating mints prove variable supply on chain; the post-revoke mint is rejected by `require_authority` (not an init side effect — the holding already exists, `mut`). Re-verify read-only via `bash scripts/demo-testnet-live.sh verify`. The prior 2026-06-03 run (ProgramId `4153e159…2caeb08f`) used the pre-fix guest and is retained as history. Corroborated at the wire/semantic layer by the 2026-05-18 local-sequencer rerun (see `docs/LEZ_PROOF_LOG.md`).
- End-to-end tests against LEZ sequencer in CI: complete as a CI-included prepared-run path, with hosted constraints disclosed. Hosted CI runs `local-sequencer-e2e-preflight` on every push/PR to report LEZ/RISC0/wallet readiness and shell-check the localnet scripts. The real standalone local-sequencer lifecycle is included as manual CI job `local-sequencer-e2e` for a prepared self-hosted runner labeled `self-hosted`, `logos-lez`, and `risc0`; it runs `scripts/preflight-localnet-e2e.sh --real-run` and then `scripts/demo-localnet.sh` with `RISC0_DEV_MODE=0`. A real M4 Pro prepared-host run passed on 2026-06-09: guest build, deploy, create_mint, create_holding, mint_to(60), mint_to(40), revoke, guard-rejected post-revoke mint, final `supply=100`, holding `balance=100`, authority `None`.
...
- Reproducible demo script: complete for offline, local-sequencer, and public-testnet paths.
  ...
  - `scripts/demo-testnet-live.sh verify` re-verifies the public-testnet deploy + lifecycle straight from the sequencer (read-only; needs only the `wallet` binary, no build or faucet); its `full` mode documents a fresh deploy from a funded account. (Its current hashes are the corrected 2026-06-27 public-testnet run.)
