...
- Reproducible demo script: complete for offline, local-sequencer, and public-testnet paths.
  ...
  - `scripts/demo-testnet-live.sh verify` re-verifies the public-testnet deploy + lifecycle straight from the sequencer (read-only; needs only the `wallet` binary, no build or faucet); its `full` mode documents a fresh deploy from a funded account. (Its current hashes are the corrected 2026-06-04 public-testnet run.)
