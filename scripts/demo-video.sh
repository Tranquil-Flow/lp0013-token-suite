...
section "SPEL-generated IDL (authoritative)"
echo "  Real 'spel generate-idl' output for the corrected guest — what 'make idl' emits."
echo "  Four instructions: create_mint, create_holding, mint_to, set_mint_authority,"
echo "  plus full account bodies. rc1 and rc3 pin generations are byte-identical."
echo ""
show "cat idl/admin-authority.idl.spel-generated.json"
pause "$SCENE_PAUSE"

section "Hand-written design reference"
echo "  Documents discriminators / execution / declared errors the generator omits."
echo "  metadata.caveats: discriminators are illustrative (dispatch is by variant index);"
echo "  args model the offline API. The spel-generated IDL is authoritative on-chain."
echo ""
...
echo "    2. Authoritative IDL generated by the real SPEL framework, plus a"
echo "       hand-written design reference for the omitted discriminators/errors."
echo "    3. Same semantics on the LEZ sequencer — create_mint, create_holding,"
echo "       two accumulating mints, revoke, and a post-revoke mint rejected by"
echo "       the authority guard (verified on the public testnet 2026-06-04)."
echo ""
